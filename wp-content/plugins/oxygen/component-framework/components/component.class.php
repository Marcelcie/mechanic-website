...
Class CT_Component {
	...
	function build_css($states, $selector, $original = array(), $is_media = false) {
		...
		if($is_repeater_child === 2) {
			return; // only one cycle is required 
		}
		
		if($is_repeater_child) {
			$hashSelector = ".oxy-dynamic-list :is([data-id=\"$selector\"], #incspec)";
		} else {
		...
		foreach ( $states as $key => $atts ) {
			...
			if ($this->options['tag'] == "ct_fancy_icon") {
				...
				if (!$is_media && $key == "original") {
					...
					$iconColor 			= isset($this->states['media'][$is_media][$key]['icon-color']) ? oxygen_vsb_get_global_color_value($this->states['media'][$is_media][$key]['icon-color']) : "";
