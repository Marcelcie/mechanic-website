...
function do_oxygen_elements($node) {
	...
	foreach ( $node as $child ) {
		...
		$content = (!empty($child['options']['ct_content'])) ? $child['options']['ct_content'] : "";

		if ($children && $content) {
			foreach ($children as $grand_child) {
				$parced_grand_child = do_oxygen_elements(array($grand_child));
				$content = str_replace("<span id=\"ct-placeholder-{$grand_child['id']}\"></span>", $parced_grand_child, $content);
			}
		}
		else if ($children) {
			$content = $children;
		...
		if ( isset($oxygen_registered_elements[ $tag ]) ) {
			...
			$html .= call_user_func( $oxygen_registered_elements[ $tag ], $options, $content, $tag );
			
			$html = apply_filters( 'do_shortcode_tag', $html, $tag, $options, array() );
		}
	}
