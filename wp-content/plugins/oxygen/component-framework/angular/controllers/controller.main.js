...
CTFrontendBuilder.controller("MainController", function($scope, $parentScope, $http, $timeout, $window, ctScopeService, $sce) {
    ...
    $scope.processImageSizes = function(id) {
        ...
        if ( 
            ...
            $parentScope.showLoadingOverlay("processImageSizes()");

            $scope.loadAttachmentSizes( $scope.component.options[id]['model']['attachment_id'], function( data ){
                
                $parentScope.hideLoadingOverlay("processImageSizes()");
                ...
                {
                    $scope.setOptionModel('attachment_size', $scope.component.options[id].size_labels[0]);
                }
                
                $scope.setOptionModel('attachment_height', data[ currentSize ].height);
                $scope.setOptionModel('attachment_width', data[ currentSize ].width);
                $scope.setOptionModel('attachment_url', data[ currentSize ].url);
            } );
