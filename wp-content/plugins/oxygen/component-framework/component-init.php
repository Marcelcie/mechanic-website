...
function oxy_print_cached_css() {
	...
	if(get_post_meta ( get_the_ID(), 'tcb2_ready', true )) {
		return;
	}
...
function oxygen_get_combined_tree($template_id, $retainInnerContent = false) {
	...
	if(isset($_REQUEST['oxy_preview_template_revision']) && is_numeric($_REQUEST['oxy_preview_template_revision'])) {
		return array();
		// JSON TODO: check if need something for revisions?
		//$json = Oxygen_Revisions::get_post_meta_db( $template, null, true, null, OBJECT, $_REQUEST['oxy_preview_template_revision'] )->meta_value;
		//$json = oxygen_safe_convert_old_shortcodes_to_json($json);
	} else {
	...
	if ($json) {
		...
		update_post_meta($template_id, 'ct_builder_json', addslashes(json_encode($components_tree)));
	}
...
add_action( 'oxygen_enqueue_frontend_scripts', 'add_web_font', 0 );
...
function ct_obfuscate_shortcode($content) {
    if (!empty($content)) {
        $pattern = '/(\")(url|src|map_address|alt|background-image|oxycode|value|name)(\":\"[^\"]*)\[oxygen ([^\]]*)\]([^\"\[\s]*)/i';
